...
source_result="${1:-results/video/poll-latest.json}"
output_dir="${2:-results/video-edit}"

if [[ ! -f "$source_result" ]]; then
  printf 'Source video result not found: %s\n' "$source_result" >&2
  exit 1
fi

source_url="$(jq -r '.video.url // empty' "$source_result")"
if [[ -z "$source_url" ]]; then
  printf 'Source video URL is missing in: %s\n' "$source_result" >&2
  exit 1
fi
...
prompt="Change only the sunflower petals from yellow to bright blue. Preserve the red toy robot, white ceramic pot, cream studio background, camera motion, and timing. Add no text."
model="xai/grok-imagine-video"
idempotency_key="media-agents-video-edit-benchmark-2026-07-18-v1"
...
curl_metrics_json="$(curl --silent --show-error --max-time 120 \
  ...
  --data "$(jq -nc --arg model "$model" --arg prompt "$prompt" --arg source_url "$source_url" \
    '{model:$model,prompt:$prompt,video:{url:$source_url}}')" \
  --dump-header "$output_dir/create.headers" \
  --output "$output_dir/create.json" \
  --write-out '%{json}' \
  "${NINEROUTER_URL%/}/v1/videos/edits")" || curl_exit=$?
...
if [[ "$status" == "done" && -n "$video_url" ]]; then
  if ! curl --silent --show-error --fail --max-time 300 \
    --output "$output_dir/xai__grok-imagine-video-edit.mp4.part" \
    "$video_url"; then
    rm -f "$output_dir/xai__grok-imagine-video-edit.mp4.part"
    printf 'Video download failed\n' >&2
    exit 1
  fi
  mv "$output_dir/xai__grok-imagine-video-edit.mp4.part" "$output_dir/xai__grok-imagine-video-edit.mp4"
fi

artifact=null
if [[ -f "$output_dir/xai__grok-imagine-video-edit.mp4" ]]; then
  artifact="\"$output_dir/xai__grok-imagine-video-edit.mp4\""
fi
...
if [[ "$status" != "done" || ! -f "$output_dir/xai__grok-imagine-video-edit.mp4" ]]; then
  exit 1
fi
